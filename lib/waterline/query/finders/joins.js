...
Joins.prototype.makeModels = function makeModels(cb) {

  var self = this;

  // If values are invalid (not an array), return them early.
  if (!self.values || !self.values.forEach) {
    self.models = self.values;
    cb(self);

  } else {

  // Make each result an instance of model
    compute.parallel.map(self.values, 8, function(value) {
      return self.modelize(value);
    }, function(err, models) {
      self.models = models;
      cb(self);
    });
